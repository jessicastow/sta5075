...
```{r}
cor(tobacco$burn,tobacco$chlorine) # -0.6234392
cor(tobacco$burn,tobacco$chlorine)^2 # Multiple R squared = 0.3886764
```


## Explain what this quantity represents and why a larger R2 is preferred. 
- R squared is the amount of variation in the dependent variable that can be explained by the independent variables
- A larger R squared is preferred since ...
...
##  Add the remaining variables to the regression model one at a time. Add the variable with the next largest (absolute) correlation coefficient between with the response variable and so forth. Take note of the correlation co- efficient between the fitted values of the response variable for the new regression equations and the response variable, the R2 value and the significance of the beta parameters.
...
```{r}
lm2 <- lm(tobacco$burn ~ tobacco$chlorine + tobacco$potas, data = tobacco)
summary(lm2)
```
...
X <- data.frame(intercept = 1, tobacco[2:4])
X <- as.matrix(X)
View(X)
Y <- tobacco$burn
